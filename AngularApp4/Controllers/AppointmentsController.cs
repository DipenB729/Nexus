...
public class AppointmentsController : ControllerBase
{
    ...
    [Authorize(Policy = "AdminOnly")]
    public async Task<ActionResult<ApiResponse<Appointment>>> Reschedule(long appointmentId, AppointmentCreateDto dto)
    {
        ...
        item.AppointmentDate = dto.AppointmentDate.Date;
        item.SlotStartTime = dto.SlotStartTime;
        item.SlotEndTime = dto.SlotEndTime;
        item.Status = AppointmentStatus.Rescheduled;
        item.AdminRemarks = "Rescheduled by admin";
        item.UpdatedAt = DateTime.UtcNow;

        await _db.SaveChangesAsync();
        return Ok(ApiResponse<Appointment>.Ok(item, "Appointment rescheduled"));
    }
